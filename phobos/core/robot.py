...
from ..utils.misc import read_number_from_config, regex_replace, create_dir, edit_name_string, execute_shell_command, get_var, plural
...
class Robot(SMURFRobot):
    def __init__(self, name=None, xmlfile=None, submechanisms_file=None, smurffile=None, verify_meshes_on_import=True,
                 ...
                 assert_validity=True, shallow=False, **kwargs):
        ...
        try:
            super().__init__(xmlfile=xmlfile, submechanisms_file=submechanisms_file, smurffile=smurffile,
                             ...
                             autogenerate_submechanisms=autogenerate_submechanisms, is_human=is_human, shallow=shallow, **kwargs)
    ...
    # export methods
    def export_meshes(self, mesh_output_dir, rel_mesh_pathes=None, format=None, use_existing=False, apply_scale=False):
        """
        ...
        for vc in self.visuals + self.collisions:
            if isinstance(vc.geometry, representation.Mesh):
                vc.geometry.provide_mesh_file(targetpath=os.path.abspath(mesh_output_dir), rel_mesh_pathes=rel_mesh_pathes, format=format,
    ...
    def export_x3d(self, outputfile, float_fmt_dict=None, reduce_meshes=1000, mark_as_autogenerated=False):  #, ros_pkg=False, copy_with_other_pathes=None, ros_pkg_name=None, mesh_format=None):
        ...
        with open(outputfile, "w") as f:
            if mark_as_autogenerated:
                f.write("<!-- generated by Phobos -->\n")
            f.write(xml_string)
            f.close()
    ...
    def export_urdf(self, outputfile, float_fmt_dict=None, ros_pkg=False, copy_with_other_pathes=False, ros_pkg_name=None, mesh_format=None, add_world_joint=None, add_default_transmissions=False, mark_as_autogenerated=False, write_material_references=True):
        """Export the mechanism to the given output file.
        If export_visuals is set to True, all visuals will be exported. Otherwise no visuals get exported.
        If export_collisions is set to to True, all collisions will be exported. Otherwise no collision get exported.
        """
        ...
        if write_material_references is not None:
            export_robot.write_material_references = write_material_references

        if add_default_transmissions:
            for j in export_robot.joints:
                if j.joint_type != "fixed":
                    export_robot.add_aggregate("transmission", representation.Transmission(
                        name=j.name,
                        transmission_type="SimpleTransmission",
                        joints=[representation.TransmissionJoint(name=j.name)],
                        actuators=[representation.Actuator(name=j.name)]
                    ))
        ...
        if add_world_joint:
            ...
            xml_string = "\n".join(xml_lines[:1] + joint_lines + xml_lines[1:])
    ...
    def export_sdf(self, outputfile, float_fmt_dict=None, ros_pkg=False, copy_with_other_pathes=None, ros_pkg_name=None, mesh_format=None, add_world_joint=None, mark_as_autogenerated=False):
        """Export the mechanism to the given output file.
        If export_visuals is set to True, all visuals will be exported. Otherwise no visuals get exported.
        If export_collisions is set to to True, all collisions will be exported. Otherwise no collision get exported.
        """
    ...
    def export_xml(self, outputdir=None, format="urdf", filename=None, float_fmt_dict=None, no_format_dir=False,
                   ...
                   enforce_zero=False, correct_inertials=False, add_world_joint=None, add_default_transmissions=False, mark_as_autogenerated=False, write_material_references=True):
        ...
        assert len(self.get_roots()) == 1, [str(r) for r in self.get_roots()]
        format = format.lower()
        ...
        if format == "urdf":
            _export_robot.export_urdf(
                ...
                float_fmt_dict=float_fmt_dict, mesh_format=mesh_format, add_world_joint=add_world_joint, add_default_transmissions=add_default_transmissions,
                mark_as_autogenerated=mark_as_autogenerated, write_material_references=write_material_references
            )
        elif format == "sdf":
            _export_robot.export_sdf(
                outputfile=model_file,
                ros_pkg=ros_pkg, copy_with_other_pathes=copy_with_other_pathes, ros_pkg_name=ros_pkg_name,
                float_fmt_dict=float_fmt_dict, mesh_format=mesh_format, add_world_joint=add_world_joint,
                mark_as_autogenerated=mark_as_autogenerated
            )
        else:
    ...
    def export_smurf(self, outputdir=None, outputfile=None, robotfile=None, filename=None,
                     check_submechs=True, with_submodel_defs=False,
                     with_meshes=True, mesh_format=None, additional_meshes=None, rel_mesh_pathes=None, mark_as_autogenerated=False):
        ...
        for annotation in self.smurf_annotation_keys:
            ...
            if hasattr(self, annotation) and getattr(self, annotation):
                ...
                if annotation == "submechanisms" or annotation == "exoskeletons":
                    ...
                    with open(os.path.join(smurf_dir, "{}_{}.yml".format(filename, annotation_name)), "w+") as stream:
                        if mark_as_autogenerated:
                            annotation_dict["_autogenerated"] = True
                        stream.write(dump_json(annotation_dict, default_style=False))
                        export_files.append(os.path.split(stream.name)[-1])
        if submechanisms != {}:
            self.submechanisms_file = os.path.join(smurf_dir, "{}_submechanisms.yml".format(filename))
            with open(self.submechanisms_file, "w+") as stream:
                if mark_as_autogenerated:
                    submechanisms["_autogenerated"] = True
        ...
        for category, annos in self.annotations.items():
            if category not in self.smurf_annotation_keys:
                with open(os.path.join(smurf_dir, "{}_{}.yml".format(filename, category)), "w+") as stream:
                    dump = {category: annos}
                    if mark_as_autogenerated:
                        dump["_autogenerated"] = True
                    stream.write(dump_json(dump, default_style=False))
                    export_files.append(os.path.split(stream.name)[-1])
        ...
        if with_submodel_defs and len(self.submodel_defs) > 0:
            ...
            with open(os.path.join(smurf_dir, "{}_submodels.yml".format(filename)), "w+") as stream:
                dump = {"submodels": out_submodel_defs}
                if mark_as_autogenerated:
                    dump["_autogenerated"] = True
                stream.write(dump_json(dump, default_style=False))
                export_files.append(os.path.split(stream.name)[-1])
    ...
    def export_joint_limits(self, outputdir, file_name="joint_limits.yml", joint_desc=None, mark_as_autogenerated=False):
        ...
        with open(os.path.join(outputdir, file_name), "w") as jl_file:
            if mark_as_autogenerated:
                output_dict["_autogenerated"] = True
    ...
    def export(self, outputdir, export_config=None, rel_mesh_pathes=None, ros_pkg_name=None, no_smurf=False, filename=None,
               ros_pkg_later=False, check_submechs=True, with_meshes=True, reduce_meshes=None, use_existing_meshes=False,
               apply_scale=False, export_null_pose=True, sort_links_and_joints=True, mark_as_autogenerated=False):
        assert self.check_linkage()
        if sort_links_and_joints:
            # this sorting is required to minimize the produced diff when tracking the model in a VCS
            self.joints = self.get_joints_ordered_df()
            self.links = self.get_links_ordered_df()
            ...
            if export["type"] in KINEMATIC_TYPES:
                ...
                if export["name"] not in main_export_robot_instance.submodel_defs:
                    export_robot_instance = main_export_robot_instance.define_submodel(
                        ...
                        remove_links=export.get("remove_links", None),
                        move_joint_axis_to_intersection=export.get("move_joint_axis_to_intersection", None),
                        name_replacements=export.get("name_replacements", None)
                    )
                else:
                ...
                if "export_config" in export:
                    ...
                    _export_config = [ec for ec in export_config if ec["type"] not in ["for-loop", "submodel"]]
                ...
                export_robot_instance.export(
                    ...
                    use_existing_meshes=use_existing_meshes,
                    no_smurf=no_smurf,
                    filename=export.get("filename", None),
                    mark_as_autogenerated=mark_as_autogenerated
                )
        ...
        if ros_pkg and not ros_pkg_later:
            main_export_robot_instance.export_ros_package_files(outputdir, ros_pkg_name, mark_as_autogenerated=mark_as_autogenerated)
        elif ros_pkg and ros_pkg_later:
            return ros_pkg_name

    def export_ros_package_files(self, outputdir, ros_pkg_name, author=None, maintainer=None, url=None, version=None, license=None, cmake=None, package_xml=None, mark_as_autogenerated=False):
        # ROS CMakeLists.txt
        ...
        if (cmake is None and not os.path.isfile(ros_cmake)) or cmake: # if we want to keep that we can put it to the keep_files
            ...
            with open(ros_cmake, "w") as cmake:
                if mark_as_autogenerated:
                    cmake.write("# generated by Phobos\n")
                cmake.write(content)
        # ROS package.xml
        ...
        if (package_xml is None and not os.path.isfile(packagexml_path)) or package_xml:
            ...
            with open(packagexml_path, "r") as packagexml:
                ...
                if license:
                    content = misc.regex_replace(content, {
                        "\$LICENSE": f"<license>"+license+"</license>"
                    })
                try:
                    ...
                    content = misc.regex_replace(content, {
                        ...
                        "\$MAINTAINER": f"<maintainer email=\"no@email.here\">{maintainer if maintainer is not None else _maintainer}</maintainer>",
            ...
            with open(packagexml_path, "w") as packagexml:
                if mark_as_autogenerated:
                    packagexml.write("<!-- generated by Phobos -->\n")
                packagexml.write(content)
        # config/ with initial poses
        config_dir = os.path.join(outputdir, "config")
        os.makedirs(config_dir, exist_ok=True)
        for pose in self.poses:
            with open(os.path.join(config_dir, f"init_positions_{pose.name}.yml"), "w") as f:
                dump = {"init_positions": pose.joints}
                if mark_as_autogenerated:
                    dump["_autogenerated"] = True
    ...
    def exchange_root(self, new_root):
        ...
        The robot should be linked before (link_entities())
        Args:
    ...
    def fix_tree(self, root, link_entities=True):
        """This method may fix your robot if it has multiple roots. This can fix it under the following conditions:
           - Your robot is serial
           - Your robot only has multiple roots because one or more joints are inverted
           - You know the one and only true root of your robot

        Args:
            root (_type_): The name of the one and only true root
        """
        self.unlink_entities()
        def _ensure_link_rootest(robot, start_link, joints_done=[]):
            # first ensure that the start_link is the rootest
            for joint in robot.joints:
                if str(joint.child) == start_link and str(joint) not in joints_done:
                    joint.invert()
                    joints_done.append(str(joint))
            return joints_done

        def _reorient_tree_below(robot, start_link, joints_done=[]):
            for joint in robot.joints:
                if joint.parent == start_link:
                    link = joint.child
                    joints_done += _ensure_link_rootest(link, joints_done=joints_done)
                    joints_done += _reorient_tree_below(link, joints_done=joints_done)
            return joints_done

        joints_done = self._ensure_link_rootest(self, str(root))
        _reorient_tree_below(self, str(root), joints_done=joints_done)
        self.regenerate_tree_maps()
        if link_entities:
            self.link_entities()
    ...
    def remove_fixed(self):
        """Removes all fixed joints, see remove_joint
        """
        remove = [str(j) for j in self.joints if j.joint_type == "fixed"]
        self.remove_joint(remove)
    ...
    def move_link_in_tree(self, link_name, new_parent_name):
        ...
        joint = self.get_joint(jointname)
        if joint.parent == new_parent_name:
            return
    ...
    def define_submodel(self, name, start=None, stop=None, robotname=None, only_urdf=False, abstract_model=False,
                        remove_joints=None, remove_links=None, remove_fixed=False, no_submechanisms=False, include_unstopped_branches=False,
                        move_joint_axis_to_intersection=None, include_human_in_abstract=False, name_replacements=None,
                        overwrite=False):
        ...
        if type(stop) == str:
            stop = [stop]
        if type(stop) == list and len(stop)==0:
            stop = None
        assert stop is None or (type(stop) == list and len(stop) > 0)
        definition = {
            "name": name,
            "robotname": robotname,
    ...
    def instantiate_submodel(self, name=None, start=None, stop=None, robotname=None, include_unstopped_branches=None,
                             no_submechanisms=False, abstract_model=False, remove_joints=None, remove_links=None, remove_fixed=False,
                             move_joint_axis_to_intersection=None, include_human_in_abstract=False, name_replacements=None,
                             **ignored_kwargs):
        """
        ...
        if remove_joints is None:
            remove_joints = []
        elif type(remove_joints) == str:
            remove_joints = [remove_joints]
        if remove_joints is None:
            remove_links = []
        elif type(remove_links) == str:
            remove_links = [remove_links]
        if start is None:
            start = self.get_root()
        if type(stop) == str:
        ...
        elif type(stop) == list and len(stop) > 0:
            _stop = []
            for link_id in stop:
                for link in self.links:
                    if re.match(link_id, str(link)):
                        _stop.append(str(link))
            stop = list(set(_stop))
        ...
        try:
            ...
            log.error(f"During instantiation of submodel {name} the following exception was raised: {e}")
            raise e
        ...
        for link in submodel.links:
            for vc in link.collisions+link.visuals+link.primitives:
        ...
        if remove_links:
            _new_remove_links = []
            for link_id in remove_links:
                for link in submodel.links:
                    if re.match(link_id, str(link)):
                        _new_remove_links.append(str(link))
            remove_links = _new_remove_links
            submodel.remove_link(list(set(remove_links)))

        _new_remove_joints = []
        for joint_id in remove_joints:
            for joint in submodel.joints:
                if re.match(joint_id, str(joint)):
                    _new_remove_joints.append(str(joint))
        remove_joints = _new_remove_joints
        ...
        if name_replacements:
            submodel.edit_names({"replacements": name_replacements})
    ...
    def correct_inertials(self, limit=0.0):
        """
        ...
        for link in self.links:
            ...
            if link.inertial:
                ...
                log.info(" Link {} has no inertial defined".format(link.name))
                continue
            m = M[0, 0]
            if m < limit:
        ...
        for joint in self.joints:
            ...
            if not hasattr(joint, "limit") or joint.limit is None:
                ...
                if backup is not None:
                    ...
                    log.warning(f"Joint limits for {joint.name} not defined taking default values!")
                elif raise_error:
                ...
                if hasattr(joint, "limit") and joint.limit is not None and (
                        (joint.limit.lower == joint.limit.upper and not joint.joint_type == "continuous") or joint.limit.velocity == 0 or joint.limit.effort == 0):
                    ...
                    if backup is not None:
                        ...
                        log.info(f"Therefore we take the backup/default values for the joint limits:\n"
                                    f"min: {joint.limit.lower} max {joint.limit.upper} vel {joint.limit.velocity} "
                                    f"eff {joint.limit.effort}")
                if any([not hasattr(joint.limit, x) for x in ["lower", "upper"]]) or \
                        any([getattr(joint.limit, x) is None for x in ["lower", "upper"]]) and joint.joint_type not in ["continuous"]:
                    log.error(f"{joint.name} has no limits defined! min:{joint.limit.lower} max:{joint.limit.upper} eff:{joint.limit.effort} "
                              f"vel:{joint.limit.velocity}")
                    result &= 2
                    if raise_error:
                        raise ValueError("ERROR: Not all joint limits for " + joint.name + " defined!")
                if (joint.joint_type == ["revolute", "prismatic", "continuous"]) and \
    ...
    def rename(self, targettype, target, prefix=None, suffix=None, replacements=None, do_not_double=True, joint_equals_link_name=False):
        ...
        if targettype.startswith("joint") and joint_equals_link_name:
            ...
            new_name = edit_name_string(target, prefix=prefix, suffix=suffix, replacements=replacements, do_not_double=do_not_double, correspondance=self.get_aggregate(targettype, target))
    ...
    def clean_meshes(self, check_enough_vertices=True):
        """
        ...
        for link in self.links:
            for vc in link.collisions:
                if isinstance(vc.geometry, representation.Mesh) and not vc.geometry.is_valid(check_enough_vertices=check_enough_vertices):
                    log.warning(f"Mesh-Geometry {vc.name} {vc.geometry.input_file} is empty/too small; removing the corresponding {repr(type(vc)).split('.')[-1][:-2]}!")
                    if getattr(vc, "primitive", []):
                        vc.geometry = None
                    else:
    ...
    def attach(self, other, joint, do_not_rename=False, name_prefix="", name_suffix="_2", link_other=False):
        ...
        new_poses = [p for p in pposes if str(p) not in pposes & cposes] + [p for p in cposes if str(p) not in pposes & cposes]
        conflicting_poses = []
        for pose_name in pposes & cposes:
            ppose = [pp for pp in self.poses if str(pp) == pose_name][0]
            cpose = [cp for cp in other.poses if str(cp) == pose_name][0]
    ...
    def mirror_model(self, mirror_plane=None, flip_axis=1, exclude_meshes=None, name_replacements=None,
        ...
        def recursive_link_transform(link_, T_root_to_link):
            ...
            for col in new_link.collisions:
                for p in col.primitives:
                    T = T_R.dot(T_link.dot(p.origin.to_matrix()))
                    p.origin = representation.Pose.from_matrix(inv(T_root_to_link).dot(T.dot(T_flip)), relative_to=new_link)
    ...
    def remove_joint(self, jointname, only_frame=True):
        ...
        If only_frame is set to the physical information of the child link is removed as well
        """
        ...
        if joint is None:
            ...
            if only_frame:
                ...
                for col in child.collisions:
                    for p in col.primitives:
                        p.origin =  representation.Pose.from_matrix(C_T_P.dot(p.origin.to_matrix()), relative_to=parent)
    ...
    def remove_link(self, linkname):
        """
        Removes the link with all its physical properties and the joint by which it was attached.
        """
        if type(linkname) != list:
            linkname = [linkname]
        for ln in linkname:
            jointname = self.get_parent(ln)
            self.remove_joint(jointname=jointname, only_frame=False)
